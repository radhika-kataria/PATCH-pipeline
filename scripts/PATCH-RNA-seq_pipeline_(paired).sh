...
trimmomatic PE \
	${sample}_F1.fq \
    ${sample}_F2.fq \
    ${sample}_trimmed_F1.fq \
    ${sample}_trimmed_F1_UP.fq \
    ${sample}_trimmed_F2.fq \
    ${sample}_trimmed_F2_UP.fq \
    LEADING:28 TRAILING:28 SLIDINGWINDOW:4:28 MINLEN:28

#Check quality of fastq file post trimming
mkdir fastqc_trim
fastqc ${sample}_trimmed_F1.fq --outdir fastqc_trim
fastqc ${sample}_trimmed_F2.fq --outdir fastqc_trim

#Align trimmed fastq files to the human ref genome GRCh38 using hisat2
hisat2 -x /scratch/users/k1802884/azure/radhika/GRCh38/index/GRCh38_index \
--known-splicesite-infile /scratch/users/k1802884/azure/radhika/GRCh38/index/GRCh38_splice_sites.txt   \
-1 ${sample}_trimmed_F1.fq -2 ${sample}_trimmed_F2.fq | samtools view -bS - > ${sample}_host_aligned.bam 

echo ${sample} Hisat2 alignment finished 

#Extract "unmapped"/ non-human reads using samtools flags
samtools view -F 4 ${sample}_host_aligned.bam > ${sample}_unmapped.bam
...
#Convert bam file to fastq format using bedtools
bedtools bamtofastq -i ${sample}_unmapped_sorted.bam -fq1 ${sample}_unmapped_F1.fq -fq2 ${sample}_unmapped_F2.fq

echo ${sample} Unmapped bam to fastq

#Denovo assemlbly of unmapped reads using SPAdes
/scratch/users/k1802884/tools/SPAdes-3.14.1-Linux/bin/rnaspades.py rnaspades.py -1 ${sample}_unmapped_F1.fq -2 ${sample}_unmapped_F2.fq --only-assembler -o spades
